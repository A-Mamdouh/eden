...
Mat4 buildWorldMatrix(
    ...
    std::unordered_map<EntityId, Mat4>& cache)
{
    ...
    const Mat4 local =
        Math::composeTransform(transform.position, transform.rotationEuler, transform.scale);

    Mat4 world = local;
    const auto &ehc = registry.get<EntityHierarchy>(entityId);
    if (ehc.parent != entt::null &&
        ehc.parent != entityId &&
        registry.valid(ehc.parent) &&
        registry.any_of<Transform>(ehc.parent))
    {
        world = buildWorldMatrix(ehc.parent, registry, cache) * local;
    }
...
Mat4 makeClipSpaceCorrection() {
  // Flip Y so engine/world space stays Y-up while Vulkan screen space is Y-down.
...
public:
  ...
  void beginFrame() override {
    //TODO: Add last width and last height members to class and check if window size changed, then call resize
  ...
  void renderScene(const Scene& scene) override {
    ...
    {
      const auto& cam = cameras.get<Camera>(entityId);
      if (cam.active)
      {
            setCamera(cam);
            cameraSet = true;
    ...
    if (!cameraSet && !cameras.empty())
    {
        ...
        const auto& cam = cameras.get<Camera>(first);
        setCamera(cam);
    }
    ...
    for (auto entityId : view)
    {
        ...
        submitDrawCommand(renderable.shape, params);
    }
}
...
private:
  ...
  void createGraphicsPipeline() {
    ...
    // Because we flip Y in clip space, keep the original clockwise winding.
