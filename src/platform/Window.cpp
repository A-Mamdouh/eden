...
namespace
{
    ...
    public:
        ...
        {
            SDL_Event event;
            auto input = Platform::getInstance().getInput();
            while (SDL_PollEvent(&event))
            {
                ...
                case SDL_MOUSEBUTTONDOWN:
                    input->setMouseButtonDown(fromSdlMouseButton(event.button.button));
                    break;
                case SDL_MOUSEBUTTONUP:
                    input->setMouseButtonUp(fromSdlMouseButton(event.button.button));
                    break;
...
std::shared_ptr<Window> createWindow(const WindowConfig& config)
{
    ...
    return std::make_shared<SdlWindow>(config);
}
