...
namespace Eden::Rendering
{
  namespace
  {
    ...
    /// mesh.frag exactly. Three vec4s have a 48-byte stride in the std430
    ...
    struct alignas(16) FrameUBO
    {
      ...
      /// Matches GLSL's uvec4: x is the active light count, yzw reserved.
    ...
    static_assert(sizeof(FrameUBO) == 176);
  ...
  VulkanRenderer::~VulkanRenderer()
  {
    ...
    if (lightBuffer_)
    {
      device_.destroyBuffer(lightBuffer_);
    }
    if (lightBufferMemory_)
    {
      device_.freeMemory(lightBufferMemory_);
    }
  ...
  {
    const bool swapchainChanged = settings.antiAliasing != currentSettings_.antiAliasing ||
                                  settings.vsync != currentSettings_.vsync;
    ...
    // Light-limit changes are applied during the next frame upload.
    if (swapchainChanged)
    {
      recreateSwapchain();
    }
  ...
  void VulkanRenderer::initVulkan()
  {
    ...
    ensureLightBufferCapacity(0);
    createFrameDescriptorPool();
    createFrameDescriptorSet();
    createScenePass(scenePass_);
  ...
  void VulkanRenderer::pickPhysicalDevice()
  {
    ...
    maxLightBufferSize_ = limits.maxStorageBufferRange;
  ...
  void VulkanRenderer::createScenePass(SceneRenderPass &pass)
  {
    destroyScenePass(pass);
    try
    {
  ...
  void VulkanRenderer::destroyScenePass(SceneRenderPass &pass)
  {
    if (!device_)
    {
      return;
    ...
    if (pass.pipeline)
    {
      device_.destroyPipeline(pass.pipeline);
    }
    if (pass.pipelineLayout)
    {
      device_.destroyPipelineLayout(pass.pipelineLayout);
    }

    for (const auto fb : pass.framebuffers)
    {
      device_.destroyFramebuffer(fb);
    }
    if (pass.renderPass)
    {
      device_.destroyRenderPass(pass.renderPass);
    }

    if (pass.depthView)
    {
      device_.destroyImageView(pass.depthView);
    }
    if (pass.depthImage)
    {
      device_.destroyImage(pass.depthImage);
    }
    if (pass.depthMemory)
    {
      device_.freeMemory(pass.depthMemory);
    }

    if (pass.colorView)
    {
      device_.destroyImageView(pass.colorView);
    }
    if (pass.colorImage)
    {
      device_.destroyImage(pass.colorImage);
    }
    if (pass.colorMemory)
    {
      device_.freeMemory(pass.colorMemory);
    }

    pass = SceneRenderPass{};
  }

  void VulkanRenderer::createDepthResources(SceneRenderPass &pass)
  {
    pass.depthFormat = findDepthFormat();
    ...
    imageInfo.format = pass.depthFormat;
    ...
    pass.depthImage = device_.createImage(imageInfo);

    const vk::MemoryRequirements memRequirements = device_.getImageMemoryRequirements(pass.depthImage);
    vk::MemoryAllocateInfo allocInfo{};
    allocInfo.allocationSize = memRequirements.size;
    allocInfo.memoryTypeIndex =
        findMemoryType(memRequirements.memoryTypeBits, vk::MemoryPropertyFlagBits::eDeviceLocal);

    pass.depthMemory = device_.allocateMemory(allocInfo);
    device_.bindImageMemory(pass.depthImage, pass.depthMemory, 0);

    vk::ImageViewCreateInfo viewInfo{};
    viewInfo.image = pass.depthImage;
    viewInfo.viewType = vk::ImageViewType::e2D;
    viewInfo.format = pass.depthFormat;
    ...
    pass.depthView = device_.createImageView(viewInfo);
  }

  void VulkanRenderer::createColorResources(SceneRenderPass &pass)
  {
    ...
    pass.colorImage = device_.createImage(imageInfo);

    const vk::MemoryRequirements memRequirements = device_.getImageMemoryRequirements(pass.colorImage);
    vk::MemoryAllocateInfo allocInfo{};
    allocInfo.allocationSize = memRequirements.size;
    allocInfo.memoryTypeIndex =
        findMemoryType(memRequirements.memoryTypeBits, vk::MemoryPropertyFlagBits::eDeviceLocal);

    pass.colorMemory = device_.allocateMemory(allocInfo);
    device_.bindImageMemory(pass.colorImage, pass.colorMemory, 0);

    vk::ImageViewCreateInfo viewInfo{};
    viewInfo.image = pass.colorImage;
    viewInfo.viewType = vk::ImageViewType::e2D;
    ...
    pass.colorView = device_.createImageView(viewInfo);
  }

  void VulkanRenderer::createRenderPass(SceneRenderPass &pass)
  {
    ...
    depthAttachment.format = pass.depthFormat;
    ...
    pass.renderPass = device_.createRenderPass(renderPassInfo);
  }
  ...
  void VulkanRenderer::createFrameDescriptorSetLayout()
  {
    ...
    const std::array<vk::DescriptorSetLayoutBinding, 2> bindings = {uboBinding, lightBinding};
  ...
  void VulkanRenderer::ensureLightBufferCapacity(std::size_t lightCount)
  {
    const auto maxCapacity = static_cast<std::size_t>(maxLightBufferSize_ / sizeof(GpuLight));
    if (lightCount > maxCapacity)
    {
      throw std::runtime_error("Frame lights exceed Vulkan maxStorageBufferRange; reduce graphics.maxLights");
    }
    // Even an empty scene needs a valid, nonzero descriptor range.
    const std::size_t required = std::max<std::size_t>(1, lightCount);
    if (required <= lightBufferCapacity_)
    {
      return;
    }
    const std::size_t capacity = std::min(maxCapacity, std::max(required, lightBufferCapacity_ * 2));
    const auto [buffer, memory] = createBuffer(capacity * sizeof(GpuLight), vk::BufferUsageFlagBits::eStorageBuffer);

    // beginFrame() has waited for the only in-flight frame before this runs.
    // Allocate first so an allocation failure leaves the previous buffer intact.
    if (lightBuffer_)
    {
      device_.destroyBuffer(lightBuffer_);
      device_.freeMemory(lightBufferMemory_);
    ...
    lightBuffer_ = buffer;
    lightBufferMemory_ = memory;
    lightBufferCapacity_ = capacity;
    if (frameDescriptorSet_)
    {
  ...
  void VulkanRenderer::updateFrameDescriptorSet()
  {
    ...
    lightInfo.range = lightBufferCapacity_ * sizeof(GpuLight);

    vk::WriteDescriptorSet lightWrite{};
    lightWrite.dstSet = frameDescriptorSet_;
    lightWrite.dstBinding = 1;
    lightWrite.descriptorType = vk::DescriptorType::eStorageBuffer;
    lightWrite.descriptorCount = 1;
    lightWrite.pBufferInfo = &lightInfo;

    const std::array<vk::WriteDescriptorSet, 2> writes = {write, lightWrite};
    device_.updateDescriptorSets(writes, {});
  }

  void VulkanRenderer::prepareFrameResources(const RenderFrame &frame)
  {
    FrameUBO ubo{};
    ...
    const std::size_t lightCount = currentSettings_.maxLights == 0
                                       ? frame.lights.size()
                                       : std::min<std::size_t>(frame.lights.size(), currentSettings_.maxLights);
    ensureLightBufferCapacity(lightCount);
    ubo.lightCount = static_cast<std::uint32_t>(lightCount);
    std::vector<GpuLight> lights(lightCount);
    for (std::size_t i = 0; i < lightCount; ++i)
    {
      const LightDesc &light = frame.lights[i];
      GpuLight &gpuLight = lights[i];
    ...
    if (!lights.empty())
    {
      uploadToBuffer(lightBufferMemory_, lights.data(), lights.size() * sizeof(GpuLight));
    }
  ...
  vk::UniqueShaderModule VulkanRenderer::createShaderModule(const std::vector<std::uint32_t> &code) const
  {
  ...
  void VulkanRenderer::createGraphicsPipeline(SceneRenderPass &pass)
  {
    ...
    pass.pipelineLayout = device_.createPipelineLayout(pipelineLayoutInfo);
    ...
    pass.pipeline = result.value;
    pass.pipelineReady = true;
  }

  void VulkanRenderer::createFramebuffers(SceneRenderPass &pass)
  {
    const bool multisampled = sampleCount() != vk::SampleCountFlagBits::e1;

    pass.framebuffers.clear();
    pass.framebuffers.reserve(swapchainImageViews_.size());
    ...
    for (const auto view : swapchainImageViews_)
    {
      ...
      const std::vector<vk::ImageView> attachments =
          multisampled ? std::vector<vk::ImageView>{pass.colorView, pass.depthView, view}
                       : std::vector<vk::ImageView>{view, pass.depthView};
      ...
      pass.framebuffers.push_back(device_.createFramebuffer(framebufferInfo));
    }
  }
      ...
      vk::DeviceSize size, vk::BufferUsageFlags usage) const
  {
    ...
    auto memory = device_.allocateMemoryUnique(allocInfo);
    device_.bindBufferMemory(*buffer, *memory, 0);

    return {buffer.release(), memory.release()};
  }
  ...
  void VulkanRenderer::recordScenePass(const SceneRenderPass &pass, const RenderFrame &frame,
                                       const FrameContext &context)
  {
    ...
    renderPassInfo.framebuffer = pass.framebuffers[context.imageIndex];
    ...
    if (pass.pipelineReady && !frame.commands.empty())
    {
      ...
      commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pass.pipelineLayout, 0,
                                       frameDescriptorSet_, {});
      ...
      for (const auto &draw : frame.commands)
      {
        ...
        commandBuffer.pushConstants(pass.pipelineLayout,
        ...
        if (baseColor && metallicRoughness && emissive)
        {
          ...
          commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, pass.pipelineLayout, 1, materialSets, {});
        }
  ...
  std::optional<VulkanRenderer::FrameContext> VulkanRenderer::beginFrame()
  {
    ...
    vk::ResultValue<std::uint32_t> acquireResult{vk::Result::eSuccess, 0};
    try
    {
      acquireResult = device_.acquireNextImageKHR(swapchain_, UINT64_MAX, imageAvailableSemaphore_, nullptr);
    }
    ...
    // A suboptimal image is still acquired: submit/present it before rebuilding
    // so imageAvailableSemaphore_ is consumed by this frame's submission.
    framebufferResized_ = acquireResult.result == vk::Result::eSuboptimalKHR;
    const std::uint32_t imageIndex = acquireResult.value;
    auto commandBuffer = commandBuffers_[imageIndex];
    commandBuffer.reset();
    vk::CommandBufferBeginInfo beginInfo{};
    beginInfo.flags = vk::CommandBufferUsageFlagBits::eOneTimeSubmit;
    commandBuffer.begin(beginInfo);
    return FrameContext{.imageIndex = imageIndex, .commandBuffer = commandBuffer};
  }

  void VulkanRenderer::endFrame(const FrameContext &context)
  {
    ...
    vk::Semaphore signalSemaphores[] = {renderFinishedSemaphores_[context.imageIndex]};
  ...
  void VulkanRenderer::renderFrame(const RenderFrame &frame)
  {
    ...
    recordScenePass(scenePass_, frame, *context);
    endFrame(*context);
  }
